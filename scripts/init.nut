...
print("Combine Engine - Squirrel Initialized!")
...
onUpdate(function(dt) {
    ...
    scriptedCube.transform.rotation.y += 60.0 * dt
    scriptedCube.transform.rotation.x = sin(totalTime() * 2.0) * 15.0

    local pulse = (sin(totalTime() * 3.0) + 1.0) * 0.5
    scriptedCube.color.r = 1.0
    scriptedCube.color.g = 0.3 + pulse * 0.4
    scriptedCube.color.b = pulse * 0.3

    if (isKeyDown(KEY_W)) {
        cam.position.z -= moveSpeed * dt
    }
    if (isKeyDown(KEY_S)) {
        cam.position.z += moveSpeed * dt
    }
    if (isKeyDown(KEY_A)) {
        cam.position.x -= moveSpeed * dt
    }
    if (isKeyDown(KEY_D)) {
        cam.position.x += moveSpeed * dt
    }
    ...
    cam.rotation.y += (targetCamY - cam.rotation.y) * 10.0 * dt
    cam.rotation.x += (targetCamX - cam.rotation.x) * 10.0 * dt
